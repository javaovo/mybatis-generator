...
public class MyBatisGeneratorConfigurationParser {
	...
	private void parseJavaServiceGenerator(Context context, Node node) {
		...
		String baseInterface = attributes.getProperty("baseInterface"); //$NON-NLS-1$
		String enableTx = attributes.getProperty("enableTx", "false"); //$NON-NLS-1$
		...
		javaServiceGeneratorConfiguration.setBaseService(baseService);
		javaServiceGeneratorConfiguration.setBaseInterface(baseInterface);
		javaServiceGeneratorConfiguration.setEnableTx(Boolean.valueOf(enableTx));
